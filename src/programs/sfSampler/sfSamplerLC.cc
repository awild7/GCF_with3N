...
// Tree variables
Double_t alphaI, ptI[2], alphaRec, ptRec[2];
Double_t weight;
Int_t lead_type, rec_type;

void Usage()
{
  cerr << "Usage: ./sfSamplerLC <Z> <N> <path/to/output.root> <# of samples>\n\n"
       << "Optional flags:\n"
       << "-v: Verbose\n"
...
bool init(int argc, char ** argv)
{
  ...
  outtree->Branch("alphaI",&alphaI,"alphaI/D");
  outtree->Branch("ptI",ptI,"ptI[2]/D");
  outtree->Branch("alphaRec",&alphaRec,"alphaRec/D");
  outtree->Branch("ptRec",ptRec,"ptRec[2]/D");
...
void evnt(int event)
{
  ...
  // Call decay function
  TVector2 vti, vtr;  
  myGen->decay_function_lc(weight,lead_type,rec_type,alphaI,vti,alphaRec,vtr);
